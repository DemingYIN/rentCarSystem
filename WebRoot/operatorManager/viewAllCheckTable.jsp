...
function checkDel(tableId){
	if(confirm("您确定要删除吗？")){
		document.forms[0].action="deleteCheckTableServlet.do";
	document.getElementById('checkId1').value=tableId;
	document.forms[0].submit();
	}
}
function subZForm(){
...
function checkShow(checkId){
	location.href='findCheckTableById.do?checkId=' + checkId ;
}
<!-- 复选框批量删除js-->
	function selectCar(obj){
	<!-- 获取所有的复选框-->
			var cust = document.getElementsByName("cbId");
			<!-- 设置全选-->
			if(obj==0){
		...
		function deleteAll(){
			...
			if(flag){
				if(confirm("删除后不能恢复，确认删除？")){
					document.forms[0].action="deleteAllCheckTableServlet.do";
					document.forms[0].submit();
				}
			}else{
				alert("至少选择一个出租单");
	...
	<body>
		<form action="findAllRentCarTable.do" method="post">
			...
			<input type="hidden" name="checkId"
				value="${requestScope.checkTable.checkId }" />
			<input type="hidden" name="checkDate"
				value="${requestScope.checkTable.checkDate }" />
			<input type="hidden" name="checkUserId"
				value="${requestScope.checkTable.checkUserId }" />
			<input type="hidden" name="rentId"
				value="${requestScope.checkTable.rentId }" />
												...
												<table width="100%" border="0" cellspacing="0"
													...
													<tr>
														...
														<td width="94%" valign="bottom">
															<span class="STYLE1"> 出租单管理列表</span>
														</td>
													</tr>
												</table>
											</td>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="1"
							...
							<tr>
								...
								<td width="10%" height="20" bgcolor="d3eaef" class="STYLE6">
									<div align="center">
										<span class="STYLE10">检查单编号</span>
									</div>
								</td>
								<td width="6%" height="20" bgcolor="d3eaef" class="STYLE6">
									<div align="center">
										<span class="STYLE10">检查事件</span>
									</div>
								</td>
								<td width="6%" height="20" bgcolor="d3eaef" class="STYLE6">
									<div align="center">
										<span class="STYLE10">检查员</span>
									</div>
								</td>
								<td width="" height="20" bgcolor="d3eaef" class="STYLE6">
									<div align="center">
										<span class="STYLE10">问题</span>
									</div>
								</td>
								<td width="" height="20" bgcolor="d3eaef" class="STYLE6">
									<div align="center">
										<span class="STYLE10">赔费</span>
									</div>
								</td>
								<td width="" height="20" bgcolor="d3eaef" class="STYLE6">
									<div align="center">
