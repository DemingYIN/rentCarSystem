...
public class CustomersDaoImpl implements CustomersDao {
	...
	private String genanSQl(Customers customers) {
		StringBuffer sb = new StringBuffer(
				"select identity,custname,sex,address,phone,career from  customers c where 1=1 ");
...
//		sb.append(this.genanSQl(customers)).append(") t where t.rn between ")
//				.append(from).append(" and ").append(to);
		sb.append(this.genanSQl(customers)).append(")t limit ").append(
	...
	public void deleteCustomers(String identity) {
		Connection conn = null;
		String sql = "delete from customers c where c.identity = ?";
		try {
			conn = JdbcUtil.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
