...
//		String sql = "insert into roles(roleid,rolename) values(seq_roles_id.nextval,?)";
	...
	public void deleteRole(int roleid) {
		...
		try {
			...
			conn.setAutoCommit(false);
			PreparedStatement ps = conn
					.prepareStatement("delete from roles_menus rm where rm.roleid=?");
			ps.setInt(1, roleid);
			ps.executeUpdate();
			PreparedStatement ps2 = conn
					.prepareStatement("delete from roles r where r.roleid=?");
			ps2.setInt(1, roleid);
			ps2.executeUpdate();
			conn.commit();
		} catch (SQLException e) {
	...
	public void updateRole(Role role, List<String> menusList) {
		...
		try {
			...
			PreparedStatement ps3 = conn
					.prepareStatement("update roles r set r.rolename=? where r.roleid=?");
