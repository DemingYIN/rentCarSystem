...
public class LogsDaoImpl implements LogsDao {
	...
	private String genanSQl(Logs logs) {
		...
		if (logs.getActionTime() != null) {
			sb.append(" and to_char(l.actiontime,'yyyy-MM-dd') like  '%").append(Tools.dateToString(logs.getActionTime(), "yyyy-mm-dd")).append("%'");
		}
		if (logs.getId()!= 0) {
...
//		sb.append(this.genanSQl(logs)).append(") t where t.rn between ")
//				.append(from).append(" and ").append(to);
		sb.append(this.genanSQl(logs)).append(")t limit ").append(
	...
	private String countSQL(Logs logs) {
		...
		if (logs.getActionTime() != null) {
			sb.append(" and to_char(u.actiontime,'yyyy-MM-dd') like  '%").append(Tools.dateToString(logs.getActionTime(), "yyyy-MM-dd")).append("%'");
		}
		if (logs.getId() != 0) {
	...
	public void deleteLogs(int id) {
		Connection conn = null;
		String sql = "delete from logs c where c.id = ?";
		try {
			conn = JdbcUtil.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, id);
