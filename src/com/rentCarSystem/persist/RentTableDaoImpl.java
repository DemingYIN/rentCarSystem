...
//				.append(to);
	...
	private String genanSQl(RentTable rentTable) {
		StringBuffer sb = new StringBuffer(
				"select tableid,imprest,shouldpayprice,price,begindate,shouldreturndate,returndate,rentflag,custid,carid,userid from  renttable r where 1=1 ");

		if (rentTable.getTableId() != null) {
			sb.append(" and r.tableid =").append(rentTable.getTableId());
		}
		if (rentTable.getBeginDate() != null) {
			sb.append(" and to_char(r.begindate,'yyyy-MM-dd') like  '%")
					.append(
							Tools.dateToString(rentTable.getBeginDate(),
									"yyyy-mm-dd")).append("%'");
		}
		if (rentTable.getShouldReturnDate() != null) {
			sb.append(" and to_char(r.shouldreturndate,'yyyy-MM-dd') like  '%")
					.append(
							Tools.dateToString(rentTable.getShouldReturnDate(),
									"yyyy-mm-dd")).append("%'");
		}
		if (rentTable.getReturnDate() != null) {
			sb.append(" and to_char(r.returndate,'yyyy-MM-dd') like  '%")
					.append(
...
//				.append(from).append(" and ").append(to);
	...
	private String countSQL(RentTable rentTable) {
		...
		if (rentTable.getBeginDate() != null) {
			sb.append(" and to_char(u.begindate,'yyyy-MM-dd') like  '%")
					.append(
							Tools.dateToString(rentTable.getBeginDate(),
									"yyyy-mm-dd")).append("%'");
		}
		if (rentTable.getShouldReturnDate() != null) {
			sb.append(" and to_char(u.shouldreturndate,'yyyy-MM-dd') like  '%")
					.append(
							Tools.dateToString(rentTable.getShouldReturnDate(),
									"yyyy-mm-dd")).append("%'");
		}
		if (rentTable.getReturnDate() != null) {
			sb.append(" and to_char(u.returndate,'yyyy-MM-dd') like  '%")
					.append(
							Tools.dateToString(rentTable.getReturnDate(),
									"yyyy-mm-dd")).append("%'");
		}
	...
	public void updateRentTbale(RentTable rentTable) {
		Connection conn = null;
		String sql = "update renttable set imprest=?,shouldpayprice=?,price=?,begindate=?,shouldreturndate=?,rentflag=? where tableid = ?";
		try {
			conn = JdbcUtil.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			...
			ps.setDate(5, Tools.utilDateToSqlDate(rentTable
					.getShouldReturnDate()));
			ps.setInt(6, rentTable.getRentFlag());
			ps.setLong(7, rentTable.getTableId());
			ps.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
			JdbcUtil.rollbackConnection(conn);
